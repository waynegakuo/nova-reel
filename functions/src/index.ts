...
export const getTmdbData = onCall(
  ...
  async (request) => {
    ...
    // Get the parameters from the client (e.g., movie ID, search query, endpoint path)
    const { mediaType, id, query, endpoint } = request.data; // Example of data sent from client
    ...
    let url = `${baseUrl}/${mediaType}/${endpoint}`;
